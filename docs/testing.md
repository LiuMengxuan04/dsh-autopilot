...
DSH Autopilot tests the smallest layer that can prove each responsibility, then finishes with a packed, keyless product path. Tests operate only in this repository and temporary directories; the sibling DSH checkout is a read-only build and dependency source.
...
| Packed product E2E | `DSH_AUTOPILOT_E2E_BROWSER_CHANNEL=msedge pnpm run test:e2e` | A packed tarball installs into a real DSH Web profile, runs a Host-only Cordis extension, and completes a deterministic two-round autonomous Goal through system Microsoft Edge. |
...
1. build DSH Autopilot and create an npm tarball;
...
- If the Edge acceptance browser cannot launch, verify the system Edge installation and run with `DSH_AUTOPILOT_E2E_BROWSER_CHANNEL=msedge`; CI provisions it with `pnpm exec playwright install --with-deps msedge`.
